...
class Day20Vis {
    ...
    fun part2(input: String, monsterInput: String): Int {
        Tile(drawFullLake(topLeft(parseTiles(input).onEach { it.shrink() }))).also {
                Monster(monsterInput).also { m ->
                        return it.noSharps() - rotAndFlipUntilMonstersFound(
                            it,
                            m
                        ) * m.noSharps()
                    }
    ...
    private fun drawTileEastOfIt(tile: Tile): String {
        return tile.lines.mapIndexed { idx, line ->
                var cur = tile
                val sb = StringBuilder()
    ...
    private fun topLeft(tiles: List<Tile>): Tile {
        return tiles.filter {
            !it.neighbors.containsKey(0) && !it.neighbors.containsKey(3) && it.neighbors.containsKey(1) && it.neighbors.containsKey(
                2
            )
        }[0]
    }
    ...
    private fun parseTiles(input: String): List<Tile> {
        return input.split("\n\n").map { Tile(it) }.also { setNeighbors(it) }
    }

    private fun multiplyCornerPieceNames(tiles: List<Tile>): Long {
        return tiles.filter { it.neighbors.size == 2 }.map { it.id.toLong() }.reduce { acc, l -> acc * l }
    }
    ...
    private fun findNeighbors(self: Tile, tiles: List<Tile>) {
        ...
        tiles.forEach { tile ->
            when {
                tile.id != self.id -> (0 until 4).asSequence().filterNot { self.neighbors.containsKey(it) }
    ...
    class Tile(input: String) {
        ...
        init {
            input.lines().also {
                    id = it[0].substring(
                            it[0].indexOf(" ") + 1, it[0].indexOf(":")
                        ).toInt()
                }.subList(1, input.lines().size).map { it.toCharArray() }.toTypedArray().also { lines = it }
        }
        ...
        private fun isSeaMonster(lakeY: Int, lakeX: Int, monster: Monster): Boolean {
            for (y in 0 until monster.height()) {
                for (x in 0 until monster.width()) {
                    if (monster.chars[y][x] == '#' && lines[lakeY + y][lakeX + x] != '#') {
                        return false
                    }
                }
            }
        ...
        fun side(dir: Int): String = when (dir) {
        ...
        override fun toString(): String = StringBuilder().apply { append("Tile $id\n") }.also {
                it.append(lines.joinToString("\n") { chars ->
                    chars.joinToString("")
                })
            }.also { it.append("\nNeighbors${neighbors.keys.joinToString(",")}") }.toString()
    }
}
